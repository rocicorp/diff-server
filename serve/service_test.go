...
func TestCheckAccess(t *testing.T) {
	...
	for i, t := range tc {
		...
		req := httptest.NewRequest("POST", fmt.Sprintf("/%s/%s/handleSync", t.accountID, t.dbName), strings.NewReader(`{"baseStateID": "00000000000000000000000000000000", "checksum": "00000000"}`))
...
func TestConcurrentAccessUsingMultipleServices(t *testing.T) {
	...
	svc1.ServeHTTP(res[0], httptest.NewRequest("POST", "/sandbox/foo/handleSync", strings.NewReader(`{"baseStateID": "00000000000000000000000000000000", "checksum": "00000000"}`)))
	svc2.ServeHTTP(res[1], httptest.NewRequest("POST", "/sandbox/foo/handleSync", strings.NewReader(`{"baseStateID": "00000000000000000000000000000000", "checksum": "00000000"}`)))
	svc1.ServeHTTP(res[2], httptest.NewRequest("POST", "/sandbox/foo/handleSync", strings.NewReader(`{"baseStateID": "00000000000000000000000000000000", "checksum": "00000000"}`)))
