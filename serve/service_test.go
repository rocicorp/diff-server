...
func TestCheckAccess(t *testing.T) {
	...
	for i, t := range tc {
		...
		req := httptest.NewRequest("POST", fmt.Sprintf("/%s/%s/sync", t.accountID, t.dbName), strings.NewReader(`{"basis": "00000000000000000000000000000000"}`))
...
func TestConcurrentAccessUsingMultipleServices(t *testing.T) {
	...
	svc1.ServeHTTP(res[0], httptest.NewRequest("POST", "/sandbox/foo/sync", strings.NewReader(`{"basis": "00000000000000000000000000000000"}`)))
	svc2.ServeHTTP(res[1], httptest.NewRequest("POST", "/sandbox/foo/sync", strings.NewReader(`{"basis": "00000000000000000000000000000000"}`)))
	svc1.ServeHTTP(res[2], httptest.NewRequest("POST", "/sandbox/foo/sync", strings.NewReader(`{"basis": "00000000000000000000000000000000"}`)))
